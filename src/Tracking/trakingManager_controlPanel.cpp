...
void trackingManager::setupGui(){
	...
	if(IM.mode == INPUT_LIVE_VIDEO && IM.grabberType == INPUT_OFXLIBDC) {
		ofxLibdc::PointGrey& cam = *((ofxLibdc::PointGrey*) IM.vidGrabber);
		lastShutter = cam.getShutter();
		lastGain = cam.getGain();
		lastExposure = cam.getExposure();
		lastGamma = cam.getGamma();
		lastBrightness = cam.getBrightness();
